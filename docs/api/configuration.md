...
- Keep `enable_admin_functions` disabled unless you explicitly want MCP clients to use admin-only dashboard or template-helper management tools.
...
- Use the template sensor helper tools only with an admin-authenticated MCP client and only after enabling `enable_admin_functions` in the integration setup or options flow.
...
- Template sensor helpers are managed through Home Assistant config entries, not through YAML file writes
...
- stable in `0.3.11`: discovery tools, typed Lovelace tools, completions, managed-dashboard resources, prompts, stateless HTTP transport, correctly placed HACS/Home Assistant brand assets, OpenAI-function-compatible tool catalog loading, merged security hardening limits, and secure-default admin tool gating
