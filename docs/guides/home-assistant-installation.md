...
Current integration version in this repository: `0.3.3`
...
- the logs show `Loaded Home Assistant MCP version 0.3.3 entry ...`
- the logs show `Home Assistant MCP server version 0.3.3 started successfully ...`
...
- Stable in `0.3.3`: discovery tools, completions, managed-dashboard resources, prompts, typed dashboard authoring tools, correctly placed HACS/Home Assistant brand assets, and broader OpenCode-compatible tool catalog loading.
- Experimental in `0.3.3`: read-only native Home Assistant Lovelace dashboard access through dedicated `hass.*` tools and `hass://lovelace/...` resources, plus read-only frontend panel discovery through `hass.list_frontend_panels`, `hass.get_frontend_panel`, and `hass://frontend/...` resources.
